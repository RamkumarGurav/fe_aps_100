...
export async function POST(req: NextRequest) {
  try {
    ...
    const { startYear, endYear, fiscalYear, status } = await req.json();
    let yearBody = { startYear, endYear, fiscalYear, status };

    const year = new Year({ ...yearBody });
    const newYear = await year.save();
    if (!newYear) {
      throw new ApiError(500, "Error while creating new year");
    }
